...
parser.add_argument('--reward', type=str, choices=['ndcg', 'recall'], default='recall')
parser.add_argument('--value', type=str, choices=['triplet', 'overall'], default='triplet')
...
opt.log_name = ''.join([datetime.datetime.now().strftime("%d-%m-%Y"), \
                        '_Bsz_', str(opt.batch_size), '_BprLr_', str(opt.bpr_lr), '_DveLr_', str(opt.dve_lr), \
                            '_BprWeightDecay_', str(opt.bpr_weight_decay), '_DveWeightDecay_', str(opt.dve_weight_decay), \
...
for epoch_num in range(start_epoch, opt.epoch):
    ...
    for i, batch in enumerate(train_dataloader):
        ...
        if opt.mode == 'sample':
            ...
            sel_idx = torch.nonzero(sel_prob_curr).squeeze()
            reg_los = (1/2)*(torch.norm(batch_user_embedding[sel_idx]) ** 2 + \
                                torch.norm(batch_item_embedding[sel_idx]) ** 2 + \
                                torch.norm(batch_neg_item_embedding[sel_idx]) ** 2) / float(batch_user_embedding[sel_idx].shape[0])
        elif opt.mode == 'dot':
        ...
        if (epoch_num * len(train_dataloader)  + i) % opt.dve_print_interval == 0:
            format_str = 'Epoch {:3d} iter {:3d} max_weight {:.4f} min_weight {:.4f} mean_weight {:.4f} reward {:.4f}'
            logger.info(format_str.format(epoch_num, i, torch.max(data_value).item(), torch.min(data_value).item(), torch.mean(data_value).item(), reward))
            min_val = torch.min(data_value).item()
            mean_val = torch.mean(data_value).item()
            max_val = torch.max(data_value).item()
            dv_file_name = 'data_value_{:.2f}_{:.2f}_{:.2f}.pkl'.format(min_val, mean_val, max_val)
            root_dir = os.path.dirname(os.path.dirname(cur_dir))
            with open(os.path.join(root_dir, 'DataValue_Plot', dv_file_name), 'wb') as f:
                pickle.dump(data_value.cpu().data.numpy().copy(), f)
    ...
    t2 = time()
    format_str = 'Epoch {:3d} [{:.1f} s]  loss={:.4f}: bpr_loss_weighted={:.4f}, bpr_loss_ori={:.4f}'
    logger.info(format_str.format(epoch_num, t2 - t1, epoch_loss, loss_log['bpr_loss_weighted'], loss_log['bpr_loss_ori']))
